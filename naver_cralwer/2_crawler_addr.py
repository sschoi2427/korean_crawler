...
addr_page = int(input("주소를 출력합니다. 페이지의 숫자를 적으세요. 예시) 1 \n"))
...
def crawler_addr(line):
    ...
    f = open(RESULT_PATH + "addr_result {}.txt".format(tm_3), 'w', encoding="UTF-8")
    ...
    for i in range(0, line) :    
        ...
        for link in soup.select('ul > li > dl > dt > a'):
            href = link.get('href')
            print(href)
            f.write(href + "\n")
        f.write("\n")
...
print("===========================================")    
crawler_addr(addr_page)


print("▲▲▲▲▲ 입력한 페이지의 모든 기사 주소를 텍스트파일로 저장했습니다. ▲▲▲▲▲")
